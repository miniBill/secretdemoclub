module Rss.Parser exposing (Error(..), ErrorWithPath, Path, errorToString, parse, parsedCodec, rssPrefix)
...
errorWithPathCodec : Codec () ErrorWithPath
errorWithPathCodec =
    Codec.record
        (\error path ->
            { error = error
        ...
        |> Codec.field .error errorCoded
        |> Codec.field .path (Codec.list Codec.string)
        |> Codec.finishRecord


errorCoded : Codec () Error
errorCoded =
    Codec.customType
        (\fInvalidXml fExpectedNodeGotText fExpectedNodeGotOtherNode fMissingField fUnexpectedAttribute fUnexpectedText fExpectedTextGotNodes fExpectedAttributeNotFound fInvalidDate value ->
            case value of
                InvalidXml ->
                    fInvalidXml

                ExpectedNodeGotText { expected, text } ->
                    fExpectedNodeGotText expected text

                ExpectedNodeGotOtherNode { expected, actual } ->
                    fExpectedNodeGotOtherNode expected actual

                MissingField f ->
                    fMissingField f.expected f.children

                UnexpectedAttribute name ->
                    fUnexpectedAttribute name

                UnexpectedText ->
                    fUnexpectedText

                ExpectedTextGotNodes ->
                    fExpectedTextGotNodes

                ExpectedAttributeNotFound { expected, attributes } ->
                    fExpectedAttributeNotFound expected attributes

                InvalidDate date ->
                    fInvalidDate date
        )
        |> Codec.variant0 InvalidXml
        |> Codec.variant2 (\expected text -> ExpectedNodeGotText { expected = expected, text = text }) Codec.string Codec.string
        |> Codec.variant2 (\expected actual -> ExpectedNodeGotOtherNode { expected = expected, actual = actual }) Codec.string Codec.string
        |> Codec.variant2 (\expected children_ -> MissingField { expected = expected, children = children_ }) Codec.string (Codec.list nodeCodec)
        |> Codec.variant1 UnexpectedAttribute Codec.string
        |> Codec.variant0 UnexpectedText
        |> Codec.variant0 ExpectedTextGotNodes
        |> Codec.variant2 (\expected attributes -> ExpectedAttributeNotFound { expected = expected, attributes = attributes }) Codec.string (Codec.list attributeCodec)
        |> Codec.variant1 InvalidDate Codec.string
        |> Codec.finishCustomType


attributeCodec : Codec e Attribute
attributeCodec =
...
nodeCodec =
    Codec.customType
        (\felement ftext value ->
            case value of
                Element name attrs nodes ->
                    felement name attrs nodes

                Text text ->
                    ftext text
        )
        |> Codec.variant3 Element Codec.string (Codec.list attributeCodec) (Codec.lazy (\_ -> Codec.list nodeCodec))
